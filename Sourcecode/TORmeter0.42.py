...
class EVENT:
    ...
    def dump_data_file(self, outfile):
    ...
    def replace_words(self, ability):
        ...
        new = ability.replace('Impact Control', 'Battle Readyness')
...
class TORmeter:
    ...
    def check_filesize(self):
        ...
        if (i > limit):
            ...
            tkMessageBox.showwarning(
                "TORmeter",
                "Your newest log contains no encounter information.\nIf you want to review an older log, please select if from the file->load log option."
                )
    ...
    def analyze_file(self):
        ...
        for line in ofile:
            ...
            if split[-1][-1] == ">" or split[-1][-1] == ")":
                ...
                #if player is damaging or healing something
                elif check or check2 and proceed:
                    
                    #Mobname not yet set and the actor is not a companion.
                    if lock != len(events) and ':' not in split[1]:

                        #Enemy player attacking you
                        if (split[2].startswith('[@' + playername) and check == True) and split[1][1] == '@':
                            mobname = split[1].strip('[@')[:-1];
                            lock = len(events);

                        
                        #You are attacking an enemy player
                        elif split[1].startswith('[@' + playername) and split[2][1] == '@' and check == True:
                            mobname = split[2][2:-1];
                            lock = len(events);
                        ...
                        #You damage NPC
                        elif split[1].startswith('[@' + playername) and split[2][1] != '@' and split[2] != '[]':
        ...
        for j in range(len(events)):
            if events[j].time == False:
                if j != (len(events)-1):
                    ...
                    events[j].warnings = "Warning: Unable to determine combat endpoint!\n"
