...
class EVENT:
    ...
    def dump_data_new(self):
        output = "%-30s %8s" % (self.mobname, self.t0)
    ...
    def update_enc_list(self):
        ...
        for i in range(len(self.events)):
            if self.events[i].time > self.min_t:
                self.load_enc_options.append(self.events[i].t0 + " " + self.events[i].mobname)
                ...
                if 'EnterCombat' in split:
                    ...
                    if ':' not in split[2]:
                        ...
                        if lock != len(events):
                            ...
                            elif (split[2].startswith('[@' + playername) and split[3].strip('[@')[:-1] != playername) and split[3][1] == '@' and check2 == False:
                                if ':' not in split[3]:
                                    mobname = split[3][2:-1];
                                   # if mobname.startswith("]"):
                                    #    print "hey4"   
                            ...
                            elif (split[2].startswith('[@' + playername) and split[3].startswith('[@' + playername) == False) and split[3][1] != '@' and split[3] != '[]':
